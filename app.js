document.addEventListener("DOMContentLoaded", function () {
    ...
    if (bestScore) {
        document.getElementById("bestscorenum").innerText = bestScore;
    }
    ...
    function createGameboard() {
        for (let i = 0; i < 18; i++) {
            ...
            imgOrGif.src = "imagesgifs/facedown.png";
    ...
    function endGame() {
        ...
        document.getElementById("gameboardrow").remove();
        let startArea = document.querySelector(".startpage");
        let restartGame = document.createElement("button");
        let winGameDiv = document.createElement("div");
        winGameDiv.setAttribute("id", "restartbtn");
        restartGame.classList.add("custom1");
        restartGame.innerText = "Restart Game";
        restartGame.setAttribute("id", "buttonrestart");
        winGameDiv.appendChild(restartGame);
        startArea.appendChild(winGameDiv);

        restartGame.addEventListener("click", function () {
            location.reload();
        })

        let bestScore = +localStorage.getItem("bestcore") || Infinity;
        if (currentScore < bestScore) {
            document.getElementById("bestscorenum").innerText = currentScore;
